...
function App() {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    axios.post('/forgot-password', { document: form.document, email: form.email })
      .then(res => {
        if (res.status === 200) {
  ...
  const handleSubmit2 = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    axios.post('/reset-password', { token: form2.token, password: form2.password, confirmPassword: form2.confirmPassword })
      .then(res => {
        if (res.status === 200) {
